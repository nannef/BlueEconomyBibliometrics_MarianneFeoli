...
Citations for the most frequently used libraries can be found in the article's references.
...
#Top ten most influential journals by altimetric score
# Calculate the influence of each journal by summing inputs across all altmetrics scores
influence_per_journalscore <- Altmetrics %>%
  group_by(journal) %>%
  summarise(total_influenceSC = sum(score)) %>%
  ungroup() %>%
  arrange(desc(total_influenceSC)) # Sort in descending order of influence
# Select the top 10 most influential journals
top_10_journalsSC <- head(influence_per_journalscore, 10)
# Plot the bar chart for the top 10 most influential journals
ggplot(top_10_journalsSC, aes(x = reorder(journal, total_influenceSC), y = total_influenceSC)) +
  geom_bar(stat = "identity", fill = "steelblue") +
  labs(x = "Journal", y = "Total Influence",
       title = "Top 10 Most Influential Journals by score",
...
Here we will use the dataset with sectors in extra rows (metaEXTRA).
...
Some quick statistics to ensure observations are significant
